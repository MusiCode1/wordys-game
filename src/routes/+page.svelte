<script lang="ts">
	...
	type ViewState = 'shelves' | 'boxes' | 'cards' | 'game';
...
{#if view === 'game'}
	<GameContainer cards={selectedCards} onExit={() => (view = 'shelves')} />
	<button
		onclick={goBack}
		class="absolute top-4 right-4 z-50 p-2 bg-white/50 hover:bg-white rounded-full transition-colors"
		title="יציאה"
	>
		✖️
	</button>
